...
def parse_doc_file(path: Path, docs_dir: Path) -> DocPage | None:
    ...
    # Keep generated discovery surfaces aligned with Just the Docs.
    if (
        frontmatter.get('nav_exclude') == 'true'
        or frontmatter.get('search_exclude') == 'true'
    ):
        return None
...
def generate_llms_txt(docs: list[DocPage], base_url: str) -> str:
    ...
    lines = [
        "<!-- Generated by `mise run docs:llms-txt`. Do not edit directly. -->",
        "",
        "# Morphir Rust",
        "",
