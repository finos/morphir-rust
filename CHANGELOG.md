...
- `morphir-mck-adapter` answers a versioned MCK capabilities request with
  exact contract version `2.0.0-draft.1` while preserving the unversioned
  request and numeric response of the version 1 exchange.
...
- IR format 3.1.0: a v3 `Specs` distribution, which publishes the module specifications of a
  ...
  (finos/morphir#970).
- The Rust MCK adapter decodes IR version 3 whole documents (`Distribution`, stripped on
  request), a v3 tree's `DistributionManifestFile` on its own, the v3 `FormatVersion` releases,
  and in the YAML profile every version 3 node it already reads in JSON (finos/morphir#970).
