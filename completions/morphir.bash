...
_morphir() {
    ...
    if ! declare -F _init_completion > /dev/null 2>&1; then
        echo >&2
        echo "Error: bash-completion is required for morphir completions but was not loaded." >&2
    ...
    local spec_file="$spec_dir/usage__usage_spec_morphir.spec"
...
cmd config subcommand_required=#true help="Inspect the effective Morphir configuration" {
    cmd show help="Show the effective configuration after merging every source" {
        flag --config help="Explicit project config file path" {
            arg <CONFIG>
        }
        flag --json help="Output as JSON"
    }
    cmd path help="Show which configuration sources were considered" {
        flag --config help="Explicit project config file path" {
            arg <CONFIG>
        }
        flag --json help="Output as JSON"
