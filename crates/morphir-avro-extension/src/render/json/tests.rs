...
fn artifact_paths_bound_long_names_with_collision_resistant_suffixes() {
    ...
    let first_path = schema_path(&first);
    let second_path = schema_path(&second);

    assert!(first_path.rsplit('/').next().unwrap().len() <= 255);
    assert!(second_path.rsplit('/').next().unwrap().len() <= 255);
    assert_ne!(first_path, second_path);

    let namespace = std::iter::repeat_n(format!("N{}", "n".repeat(250)), 5)
        .collect::<Vec<_>>()
        .join(".");
    let deep = AvroFullName::new(namespace, "Record".to_owned()).unwrap();
    assert!(schema_path(&deep).len() <= 1_024);
}
