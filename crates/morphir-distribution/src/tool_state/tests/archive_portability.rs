...
#[test]
fn zip_unicode_aliased_parent_components_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.zip");
    write_zip(
        ...
        &[
            ("desktop.exe", b"desktop"),
            ("Caf\u{e9}/helper.dll", b"helper"),
            ("Cafe\u{301}/config.json", b"config"),
        ],
    );
    let bytes = fs::read(&download).unwrap();
    let resolved = crate::ResolvedTrustedToolArtifact::test_fixture(
...
#[test]
fn tar_gzip_unicode_aliased_parent_components_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.tar.gz");
    write_tar_gzip(
...
#[test]
fn zip_file_parent_conflicts_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.zip");
    write_zip(
...
#[test]
fn tar_gzip_file_parent_conflicts_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.tar.gz");
    write_tar_gzip(
...
#[test]
fn zip_full_case_fold_aliases_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.zip");
    write_zip(
        ...
        &[
            ("desktop.exe", b"desktop"),
            ("s/helper.dll", b"helper"),
            ("\u{17f}/config.json", b"config"),
        ],
    );
    let bytes = fs::read(&download).unwrap();
    let resolved = crate::ResolvedTrustedToolArtifact::test_fixture(
...
#[test]
fn tar_gzip_full_case_fold_aliases_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.tar.gz");
    write_tar_gzip(
...
#[test]
fn zip_windows_uppercase_aliases_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.zip");
    write_zip(
        ...
        &[
            ("desktop.exe", b"desktop"),
            ("I/helper.dll", b"helper"),
            ("\u{131}/config.json", b"config"),
        ],
    );
    let bytes = fs::read(&download).unwrap();
    let resolved = crate::ResolvedTrustedToolArtifact::test_fixture(
...
#[test]
fn tar_gzip_windows_uppercase_aliases_are_rejected_before_extraction() {
    let root = tempfile::tempdir().unwrap();
    let home = MorphirHome::resolve_from(Some(root.path().join("home").as_os_str()), None).unwrap();
    let download = root.path().join("desktop.tar.gz");
    write_tar_gzip(
