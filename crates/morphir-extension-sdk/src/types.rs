...
///
/// Reject these rather than silently losing the root and deriving different
/// module names for the same files. Supply the root in [`SourceSet::root`].
...
pub struct CompileOptions {
    ...
    /// serialization. Deserialization also rejects both legacy root keys:
    /// a source root belongs in [`SourceSet::root`].
    pub extra: HashMap<String, serde_json::Value>,
}
...
impl<'de> Deserialize<'de> for CompileOptions {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let wire = Options::deserialize(deserializer)?;
...
///
/// Sources must use the `sources` envelope, which carries documents and their
/// optional root. Morphir CLI 0.4.0-beta.6 and later send this shape.
/// Unknown members, including a top-level `documents`, are ignored; they
/// cannot substitute for the required `sources` field.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    /// deserialized on their own reject it too.
    #[test]
