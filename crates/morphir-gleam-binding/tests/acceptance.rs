...
async fn project_has_structure(
    w: &mut GleamTestWorld,
    step: &cucumber::gherkin::Step,
) {
...
async fn module_should_have_values(w: &mut GleamTestWorld, name: String, count: usize) {
    let module = w.parsed_modules.iter().find(|m| m.name == name)
...
async fn convert_from_ir_v4(w: &mut GleamTestWorld, output_file: String) {
    ...
    w.generated_files.push((output_file, "// Generated".to_string()));
}
...
async fn roundtrip_should_complete(w: &mut GleamTestWorld) {
    ...
    assert!(!w.parsed_modules.is_empty(), "No modules parsed for roundtrip");
    // Check that we have generated files
    assert!(!w.generated_files.is_empty(), "No files generated for roundtrip");
}
...
async fn modules_should_be_equivalent(w: &mut GleamTestWorld) {
    ...
    assert_eq!(original.types.len(), generated.types.len(), "Type count should match");
    assert_eq!(original.values.len(), generated.values.len(), "Value count should match");
}
...
async fn roundtrip_should_preserve(w: &mut GleamTestWorld, _aspect: String) {
    ...
    assert!(!w.parsed_modules.is_empty(), "No modules to check preservation");
}
...
async fn convert_to_ir_v4(w: &mut GleamTestWorld) {
    ...
    for module_ir in &w.parsed_modules {
        ...
        if let Err(e) = visitor.visit_module_v4(module_ir) {
            w.parse_errors.push(format!("Failed to convert to IR V4: {}", e));
        }
    }
}
...
async fn i_have_temp_test_project(w: &mut GleamTestWorld) {
    let ctx = cli_helpers::CliTestContext::new()
...
async fn i_have_gleam_project_structure(
    w: &mut GleamTestWorld,
    step: &cucumber::gherkin::Step,
) {
    let ctx = w.cli_context.as_mut()
        .expect("CLI context not initialized");
...
async fn i_have_invalid_morphir_toml(
    w: &mut GleamTestWorld,
    step: &cucumber::gherkin::Step,
) {
    let ctx = w.cli_context.as_mut()
...
async fn i_have_compiled_ir(w: &mut GleamTestWorld, ir_path: String) {
    ...
    let ctx = w.cli_context.as_ref()
        .expect("CLI context not initialized");
    
    let full_path = ctx.project_root.join(&ir_path);
    // Create directory structure
    std::fs::create_dir_all(&full_path)
        .expect("Failed to create IR directory");
...
async fn i_run_cli_command(
    ...
    let result = ctx.execute_cli_command(&args)
...
async fn cli_command_should_succeed(w: &mut GleamTestWorld) {
    let result = w.cli_result.as_ref()
        .expect("No CLI result available");
...
async fn error_output_should_contain_alt(
    w: &mut GleamTestWorld,
    text1: String,
    text2: String,
) {
...
async fn cli_should_create_compile_structure(w: &mut GleamTestWorld) {
    ...
    let project = "test";
    let compile_dir = ctx.morphir_dir
        .join("out")
        .join(project)
        .join("compile")
        .join("gleam");
...
async fn cli_should_create_generate_structure(w: &mut GleamTestWorld) {
    ...
    let project = "test";
    let generate_dir = ctx.morphir_dir
        .join("out")
        .join(project)
        .join("generate")
        .join("gleam");
...
async fn json_output_should_contain(w: &mut GleamTestWorld, key: String) {
    ...
    let json: serde_json::Value = result.assert_json_output()
        .expect("JSON output is not valid");
