...
use crate::extism::host_functions::MorphirHostFunctions;
...
fn wasm_pages_for_bytes(bytes: u64) -> Result<u32, HostError> {
    ...
    let pages = bytes
        .checked_div(WASM_PAGE_BYTES)
        .ok_or_else(|| HostError::Invalid("WebAssembly page size must be non-zero".to_string()))?;
...
fn ensure_payload_within_limit(direction: &str, payload_bytes: usize) -> Result<(), HostError> {
...
impl ExtensionContainer {
        ...
        B: AsRef<[u8]> + Send + 'static,
    {
        ...
        .await
        .map_err(|error| {
            HostError::Invalid(format!("Extension discovery worker failed: {error}"))
        })?
    }
    ...
    fn from_bytes_with_limits(
        ...
        let mut plugin = PluginBuilder::new(manifest)
            ...
            .build()
            .map_err(|e| HostError::Invalid(format!("Failed to create plugin: {}", e)))?;

        // Query extension info
        let info: ExtensionInfo = {
            let output = plugin
                .call::<&[u8], &[u8]>("morphir_extension_info", &[])
                .map_err(|e| HostError::Invalid(format!("Failed to get extension info: {}", e)))?;
    ...
    pub async fn call<I: Serialize, O: DeserializeOwned>(
        ...
        let value = validate_envelope(response, id).map_err(|error| match error {
            EnvelopeError::Rpc(error) | EnvelopeError::Invalid(error) => error,
        })?;
        Ok(serde_json::from_value(value)?)
    }

    /// Call a raw function on the plugin (no JSON-RPC wrapping)
    pub async fn call_raw(&self, func_name: &str, input: &[u8]) -> Result<Vec<u8>, HostError> {
        ...
        tokio::task::spawn_blocking(move || {
            let output = plugin
                ...
                .map_err(|e| HostError::Invalid(format!("Plugin call failed: {}", e)))?;
            ensure_payload_within_limit("response", output.len())?;
            Ok(output.to_vec())
        })
        .await
        .map_err(|error| HostError::Invalid(format!("Extension plugin worker failed: {error}")))?
    }
}
...
impl ExtensionContainerBuilder {
    ...
    pub fn build(self) -> Result<ExtensionContainer, HostError> {
        ...
        if let Some(path) = self.wasm_path {
            ...
            Err(HostError::Invalid("No WASM path or bytes provided".into()))
        }
    }
}
