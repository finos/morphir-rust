...
use morphir_design::{discover_config, load_config_context, resolve_generate_output, ensure_morphir_structure};
...
use crate::output::{OutputFormat, GenerateOutput, Diagnostic, write_output};
...
pub async fn run_generate(
    ...
    use crate::output::{OutputFormat, GenerateOutput, write_output};
    ...
    let extension = registry.find_extension_by_target(&target_lang).await
        .ok_or_else(|| anyhow::anyhow!("No extension found for target: {}", target_lang))?;
    
    // Load IR (detect format)
    let ir_data = load_ir(&input_path)
        .context("Failed to load Morphir IR")?;
    ...
    let result: serde_json::Value = extension.call("morphir.backend.generate", generate_params).await
