...
The following commands are provided by the morphir launcher script, not the morphir binary itself. They manage your morphir installation.

- [`morphir self <COMMAND>`](self.md) - Manage morphir installation
- `morphir +<VERSION> <COMMAND>` - Run with a specific version
- `morphir --dev <COMMAND>` - Run from local source (dev mode)

See [morphir self](self.md) for complete documentation on version management and dev mode.
