...
`check:kit`. Temporary legacy IR gates were removed at the parent IR-4 cutover.
...
## IR cutover and retained package paths

The native gate replaces the temporary TypeScript IR parity task and its legacy
report checker. CI runs the released native CLI against this repository's adapter
on Linux, macOS and Windows. Historical comparison evidence and the cutover
conditions are recorded in [the parent migration record](https://github.com/finos/morphir/blob/main/spec/mck/migration.md).

Use `mise run check:kit` for local IR conformance. Remove custom calls to the
retired `check:kit-legacy` task and read the consolidated draft report produced by
the native gate. The adapter, its protocol tests, package adapter protocols and
`morphir-package` tests remain. Package runner migration belongs to
[finos/morphir#852](https://github.com/finos/morphir/issues/852).
