...
- **Declaration order is source order.** Modules, types and constructors are
  written in the order the source declares them. morphir-elm builds them from a
  `Dict`, so it writes them sorted by Morphir name. The two documents describe
  the same package; they list it differently.
