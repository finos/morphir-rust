...
async fn load_root<R: AsRef<[u8]>>(
    ...
    loop {
        ...
        //   example, Y may be 2^10.
            ...
            root_updates < max_root_updates,
            error::MaxUpdatesExceededSnafu { max_root_updates }
        );
        let path = format!("{}.root.json", root.signed.version.successor());
            ...
            Ok(stream) => {
                ...
                // metadata (version N) incremented by one, that is precisely N+1.
                    ...
                    root.signed.version < new_root.signed.version
                        && root.signed.version.successor() == new_root.signed.version,
                    error::OlderMetadataSnafu {
                        role: RoleType::Root,
                        current_version: root.signed.version.clone(),
                        new_version: new_root.signed.version.clone()
                    }
                );
...
async fn load_timestamp(
    ...
    // 4.6. The meta component must contain exactly one entry, snapshot.json
        ...
        error::TimestampMetaLengthSnafu {
            version: timestamp.signed.version.clone(),
        ...
        .map(|b| serde_json::from_slice::<Signed<Timestamp>>(&b))
    {
        if root.signed.verify_role(&old_timestamp).is_ok() {
                ...
                error::OlderMetadataSnafu {
                    role: RoleType::Timestamp,
                    current_version: old_timestamp.signed.version.clone(),
            ...
            // 4.6 trusted timestamp meta must have one entry, snapshot.json
                ...
                error::TimestampMetaLengthSnafu {
                    version: old_timestamp.signed.version.clone(),
            ...
            // (rollback attack to fetch older snapshot object)
                ...
                error::OlderSnapshotInTimestampSnafu {
                    snapshot_new: snapshot_meta.version.clone(),
                    timestamp_new: timestamp.signed.version.clone(),
                    snapshot_old: old_snapshot_meta.version.clone(),
                    timestamp_old: old_timestamp.signed.version.clone(),
                }
            );
        }
    }
...
async fn load_snapshot(
    ...
    // (We already checked the hash in `fetch_sha256` above.)
        ...
        error::VersionMismatchSnafu {
            role: RoleType::Snapshot,
            fetched: snapshot.signed.version.clone(),
            expected: snapshot_meta.version.clone()
        }
    );
    ...
    // 4.4 Check that snapshot.meta contains at least targets.json
        ...
        SnapshotTargetsMetaMissingSnafu {
            version: snapshot.signed.version.clone(),
        }
    );
        ...
        .map(|b| serde_json::from_slice::<Signed<Snapshot>>(&b))
    {
        ...
        if root.signed.verify_role(&old_snapshot).is_ok() {
                ...
                error::OlderMetadataSnafu {
                    role: RoleType::Snapshot,
                    current_version: old_snapshot.signed.version.clone(),
                    new_version: snapshot.signed.version.clone()
                }
            );
            ...
            // Ensure that the trusted snapshot has at least targets.json
                ...
                error::SnapshotTargetsMetaMissingSnafu {
                    version: old_snapshot.signed.version.clone(),
                }
            );
            for (name, meta) in &old_snapshot.signed.meta {
                    ...
                    error::SnapshotRoleMissingSnafu {
                        role: name,
                        old_version: old_snapshot.signed.version.clone(),
                        new_version: snapshot.signed.version.clone(),
                    }
                );
                let new_meta = snapshot.signed.meta.get(name).unwrap();
                    ...
                    error::SnapshotRoleRollbackSnafu {
                        role: name,
                        old_role_version: meta.version.clone(),
                        old_snapshot_version: old_snapshot.signed.version.clone(),
                        new_role_version: new_meta.version.clone(),
                        new_snapshot_version: snapshot.signed.version.clone(),
                    }
                );
            }
            if let Some(old_targets_meta) = old_snapshot.signed.meta.get("targets.json") {
                let targets_meta =
                    ...
                    error::OlderMetadataSnafu {
                        ...
                        current_version: old_targets_meta.version.clone(),
                        new_version: targets_meta.version.clone(),
                    }
                );
            }
        }
...
async fn load_targets(
    ...
    // 5.6.4. Check against the snapshot role's targets version
        ...
        error::VersionMismatchSnafu {
            role: RoleType::Targets,
            fetched: targets.signed.version.clone(),
            expected: targets_meta.version.clone()
        }
    );
...
async fn load_delegations(
    ...
    for delegated_role in &delegation.roles {
        ...
        delegation
            ...
            error::VersionMismatchSnafu {
                role: RoleType::Targets,
                fetched: role.signed.version.clone(),
                expected: role_meta.version.clone()
            }
        );
