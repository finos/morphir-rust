...
fn delegated_key_path() -> PathBuf {
    test_data().join("targetskey.pkcs8")
}
...
async fn base_editor() -> RepositoryEditor {
    ...
    editor
        ...
        .unwrap()
        .targets_version(NonZeroU64::new(1).unwrap().into())
        .unwrap()
        .snapshot_expires(Timestamp::now() + days(21))
        .snapshot_version(NonZeroU64::new(1).unwrap().into())
        .timestamp_expires(Timestamp::now() + days(3))
        .timestamp_version(NonZeroU64::new(1).unwrap().into());
    editor
}
...
async fn write_preserves_unmodified_delegated_role_bytes() {
    ...
    editor
        .targets_expires(Timestamp::now() + days(13))
        .unwrap()
        .targets_version(NonZeroU64::new(2).unwrap().into())
        .unwrap()
        .snapshot_expires(Timestamp::now() + days(21))
        .snapshot_version(NonZeroU64::new(2).unwrap().into())
        .timestamp_expires(Timestamp::now() + days(3))
        .timestamp_version(NonZeroU64::new(2).unwrap().into());
    let signed = editor.sign(targets_key).await.unwrap();
